...
;(function() {
  jQuery.fn.wpv_clone = function() {
    const result = jQuery.fn.clone.apply(this, arguments),
...
WPViews.ViewFrontendUtils = function($) {
  ...
  self.extract_url_query_parameters = function(query_string) {
    const query_string_pairs = {}
    if (query_string == '') {
      return query_string_pairs
    }
    const query_string_split = query_string.split('&'),
      query_string_split_length = query_string_split.length
    for (let i = 0; i < query_string_split_length; ++i) {
      const qs_part = query_string_split[i].split('=')
      if (qs_part.length != 2) {
        continue
      }
      let thiz_key = qs_part[0],
  ...
  self.get_extra_url_query_parameters_by_form = function(form) {
    const query_string = self.extract_url_query_parameters(
    ...
    for (const prop in query_string) {
      const $matchExisting = form.find(
        '[name="' + prop + '"], [name="' + prop + '\\[\\]"]'
      )
      if (
        ...
        (force_from_form ||
          ...
          $matchExisting.filter('.js-wpv-extra-url-param').length > 0)
      ) {
  ...
  self.set_extra_url_query_parameters_by_form = function(form) {
    const extra = self.get_extra_url_query_parameters_by_form(form)
    $.each(extra, function(key, value) {
      if (
        form.find('[name="' + key + '"], [name="' + key + '\\[\\]"]').length ===
        0
      ) {
        // @hack alert!! WE can not avoid this :-(
        const pieces = value.split('##URLARRAYVALHACK##'),
        ...
        if (pieces_length < 2) {
          ...
          for (let iter = 0; iter < pieces_length; iter++) {
            $('<input>')
              .attr({
                type: 'hidden',
                name: key + '[]',
  ...
  self.render_frontend_datepicker = function() {
    $(
      ...
      const thiz = $(this)
      thiz.addClass('js-wpv-frontend-datepicker-inited').datepicker({
        onSelect: function(dateText, inst) {
          let url_param = thiz.data('param'),
            data = 'date=' + dateText,
            form = thiz.closest('form')
          data +=
            '&date-format=' +
            $('.js-wpv-date-param-' + url_param + '-format').val()
          data += '&action=wpv_format_date'
          $.post(wpv_pagination_local.front_ajaxurl, data, function(response) {
            response = $.parseJSON(response)
            form
              .find('.js-wpv-date-param-' + url_param)
              .html(response['display'])
            form.find('.js-wpv-date-front-end-clear-' + url_param).show()
            form
              .find('.js-wpv-date-param-' + url_param + '-value')
  ...
  self.render_frontend_media_shortcodes = function(container) {
    ...
    container.find('.wp-video-shortcode.mejs-container').each(function() {
      const $oldStructure = $(this),
        $videoTag = $oldStructure.find(
          'video.wp-video-shortcode[id$=_from_mejs]'
        )
  ...
  self.get_form_element_type = function(selector) {
    if (selector.length > 0) {
      return selector[0].tagName == 'INPUT'
        ? selector[0].type.toLowerCase()
  ...
  self.get_updated_query_results = function(view_number, page, form, expect) {
    ...
    if (parametric_data['environment'].current_post_id > 0) {
      environment['wpv_aux_current_post_id'] =
        parametric_data['environment'].current_post_id
    }
    if (parametric_data['environment'].parent_post_id > 0) {
      environment['wpv_aux_parent_post_id'] =
        parametric_data['environment'].parent_post_id
    }
    if (parametric_data['environment'].parent_term_id > 0) {
      environment['wpv_aux_parent_term_id'] =
        parametric_data['environment'].parent_term_id
    }
    if (parametric_data['environment'].parent_user_id > 0) {
      environment['wpv_aux_parent_user_id'] =
        parametric_data['environment'].parent_user_id
    }
    ...
    if (
      ...
      search['dps_general'] = form
        .find('.js-wpv-filter-trigger, .js-wpv-filter-trigger-delayed')
        .serializeArray()
    }
    ...
    switch (parametric_data.query) {
      ...
      default:
        ...
        if (form.attr('data-targetid')) {
          ...
          $('.js-wpv-form-only.js-wpv-filter-form-' + view_number).length > 0
        ) {
          data['target_id'] = $(
            '.js-wpv-form-only.js-wpv-filter-form-' + view_number
          ).data('targetid')
        }
...
WPViews.ViewSorting = function($) {
  ...
  self.get_sort_data = function(view_number, form) {
    ...
    if (form.find('.js-wpv-sort-control-orderby').length > 0) {
      const orderby_type = WPViews.view_frontend_utils.get_form_element_type(
        form.find('.js-wpv-sort-control-orderby')
      )
      switch (orderby_type) {
        case 'select':
          sort['wpv_sort_orderby'] = form
        ...
        case 'radio':
          sort['wpv_sort_orderby'] = form
            ...
            .val()
          if (
            form.find('.js-wpv-sort-control-orderby:checked').data('orderbyas')
          ) {
            sort['wpv_sort_orderby_as'] = form
              .find('.js-wpv-sort-control-orderby:checked')
              .data('orderbyas')
    ...
    if (form.find('.js-wpv-sort-control-order').length > 0) {
      var order_type = WPViews.view_frontend_utils.get_form_element_type(
        form.find('.js-wpv-sort-control-order')
      )
      switch (order_type) {
        case 'select':
  ...
  self.set_sort_data = function(form, sort) {
    self
      .set_sort_data_inside_form(form, sort)
    ...
    if (form.find('.js-wpv-sort-control-orderby').length > 0) {
      ...
      switch (orderby_type) {
        ...
        case 'radio':
          ...
          if (
            form.find('.js-wpv-sort-control-orderby:checked').data('forceorder')
          ) {
            sort['order'] = form
              .find('.js-wpv-sort-control-orderby:checked')
              .data('forceorder')
  ...
  self.set_sort_data_orderby_select = function(orderby_control, sort) {
    if (
      orderby_control.find('option[value="' + sort['orderby'] + '"]').length ==
      0
    ) {
      orderby_control.append(
        $('<option>', {
          value: sort['orderby'],
          text: sort['orderby'],
        })
  ...
  self.set_sort_data_orderby_radio = function(orderby_control, sort, form) {
    if (
      orderby_control.filter('[value="' + sort['orderby'] + '"]').length == 0
    ) {
      form.append(
        $('<input>', {
          type: 'radio',
    ...
    form
      .find('.js-wpv-sort-control-orderby[value="' + sort['orderby'] + '"]')
      .prop('checked', true)
    if (_.has(sort, 'orderby_as')) {
      form
        .find('.js-wpv-sort-control-orderby:checked')
  ...
  self.set_sort_data_orderby_list = function(orderby_control, sort) {
    const orderby_control_to_set = orderby_control.find(
      '.js-wpv-sort-list-orderby[data-orderby="' + sort['orderby'] + '"]'
    )
    if (orderby_control_to_set.length == 0) {
      ...
      if (
        ...
        orderby_control.find('.js-wpv-sort-list-item').css({ display: 'none' })
        orderby_control_to_set
          .closest('.js-wpv-sort-list-item')
  ...
  self.set_sort_data_order_select = function(order_control, sort) {
    if (
      order_control.find('option[value="' + sort['order'] + '"]').length == 0
    ) {
      order_control.append(
        $('<option>', {
          value: sort['order'],
          text: sort['order'],
        })
    ...
    if (
      ...
      let order_labels = order_control.data('labels'),
  ...
  self.set_sort_data_order_radio = function(order_control, sort, form) {
    ...
    form
      .find('.js-wpv-sort-control-order[value="' + sort['order'] + '"]')
      .prop('checked', true)
    ...
    if (
      ...
      if (_.has(order_labels_to_push, 'asc')) {
        ...
        label_to_update.html(order_labels_to_push.asc).prepend(input_to_keep)
      }
      if (_.has(order_labels_to_push, 'desc')) {
        label_to_update = order_control
          .filter('[value="desc"]')
          .closest('label')
        input_to_keep = order_control.filter('[value="desc"]').detach()

        label_to_update.html(order_labels_to_push.desc).prepend(input_to_keep)
      }
    }
    return self
  }
  ...
  self.set_sort_data_order_list = function(order_control, sort) {
    const order_control_to_set = order_control.find(
      '.js-wpv-sort-list-order[data-order="' + sort['order'] + '"]'
    )
    if (order_control_to_set.length == 0) {
      ...
      if (
        ...
        order_control
          .find('span.wpv-sort-list-current')
          .removeClass('wpv-sort-list-current')
        order_control.find('.js-wpv-sort-list-item').css({ display: 'none' })
        order_control_to_set
          .closest('.js-wpv-sort-list-item')
          .addClass('wpv-sort-list-current')
          .css({ display: 'block' })
  ...
  self.set_sort_data_inside_form = function(form, sort) {
    if (_.has(sort, 'orderby')) {
      if (form.find('.js-wpv-sort-control-orderby').length > 0) {
        const orderby_control = form.find('.js-wpv-sort-control-orderby'),
          orderby_type = WPViews.view_frontend_utils.get_form_element_type(
            orderby_control
          )
        switch (orderby_type) {
          case 'select':
      ...
      if (form.find('.js-wpv-sort-list-orderby-dropdown').length > 0) {
        const orderby_list_control = form.find(
          '.js-wpv-sort-list-orderby-dropdown'
        )
    ...
    if (_.has(sort, 'order')) {
      if (form.find('.js-wpv-sort-control-order').length > 0) {
        const order_control = form.find('.js-wpv-sort-control-order'),
          order_type = WPViews.view_frontend_utils.get_form_element_type(
            order_control
          )
        switch (order_type) {
          case 'select':
      ...
      if (form.find('.js-wpv-sort-list-order-dropdown').length > 0) {
        const order_list_control = form.find('.js-wpv-sort-list-order-dropdown')
  ...
  self.set_sort_data_outside_form = function(form, sort) {
    const view_number = form.data('viewnumber')
    ...
    if (_.has(sort, 'order')) {
      ...
      if (extra_order.length > 0) {
        const order_type = WPViews.view_frontend_utils.get_form_element_type(
          extra_order
        )
        switch (order_type) {
          case 'select':
  ...
  $(document).on('click', '.js-wpv-sort-trigger', function(e) {
    ...
    let thiz = $(this),
      ...
      sort = {
        orderby:
          typeof thiz.data('orderby') != 'undefined'
            ? thiz.data('orderby')
            : '',
        order:
          typeof thiz.data('order') != 'undefined' ? thiz.data('order') : '',
        orderby_as:
          typeof thiz.data('orderbyas') != 'undefined'
            ? thiz.data('orderbyas')
            : '',
      },
      form_each
    form.each(function() {
      ...
      WPViews.view_frontend_utils.set_extra_url_query_parameters_by_form(
        form_each
      )
    })
    if (
      ...
      let data_for_events = {
        ...
        form: form,
        force_results_update: form.hasClass(
          'js-wpv-ajax-results-submit-enabled'
        ),
      }

      data_for_events = WPViews.view_parametric_search.extend_wpv_parametric_search_triggered_data(
        data_for_events
      )

      $(document).trigger('js_event_wpv_parametric_search_triggered', [
        data_for_events,
      ])
    }
  })
  ...
  $(document).on('click', '.js-wpv-sort-list-orderby', function(e) {
    ...
    orderby_list
      .find('span.wpv-sort-list-current')
      .removeClass('wpv-sort-list-current')
    orderby_list.find('.js-wpv-sort-list-item').css({ display: 'none' })
    orderby_list_item
      .addClass('wpv-sort-list-current')
      .css({ display: 'block' })
  ...
  $(document).on('click', '.js-wpv-sort-list-order', function(e) {
    ...
    order_list
      .find('span.wpv-sort-list-current')
      .removeClass('wpv-sort-list-current')
    order_list.find('.js-wpv-sort-list-item').css({ display: 'none' })
    order_list_item.addClass('wpv-sort-list-current').css({ display: 'block' })
  ...
  $(document).on('mouseenter', '.js-wpv-sort-list-dropdown', function() {
    const orderby_list = $(this)
    orderby_list.find('.js-wpv-sort-list-item').css({ display: 'block' })
  })
  ...
  self.adjust_sort_list_width = function(selector) {
    let sort_list = selector,
      sort_list_width = 0,
      sort_list_items = sort_list.find('.js-wpv-sort-list-item'),
      sort_list_anchors = sort_list.find('.wpv-sort-list-anchor span')

    sort_list_anchors.each(function() {
      const item_width = $(this).text().length
      if (item_width > sort_list_width) {
        sort_list_width = item_width
      }
    })
    sort_list_width =
      sort_list_width < 10 ? sort_list_width + 1 : sort_list_width
    sort_list.css({ width: sort_list_width + 'em' })
    sort_list_items.css({ width: sort_list_width + 'em' })
  }
  ...
  self.maybe_init_sort_data = function(selector) {
    ...
    selector.find('.js-wpv-sort-list-dropdown').each(function() {
      const sorting_list = $(this),
        sorting_list_type = sorting_list.hasClass(
          'wpv-sort-list-orderby-dropdown'
        )
          ? 'orderby'
          : 'order'
      ...
      switch (sorting_list_type) {
        case 'orderby':
          var sorting_current = sorting_list.find(
    ...
    selector.find('select.js-wpv-sort-control-orderby').each(function() {
      const sorting_control = $(this)
      if (sorting_control.closest('.js-wpv-filter-form').length == 0) {
        const sorting_viewnumber = sorting_control.data('viewnumber'),
          sorting_form = $('.js-wpv-filter-form-' + sorting_viewnumber),
          sorting_data = {
        ...
        if (sorting_control.find(':selected').data('orderbyas') !== undefined) {
          sorting_data['orderby_as'] = sorting_control
            .find(':selected')
            .data('orderbyas')
        }
...
WPViews.ViewPagination = function($) {
  ...
  self.add_view_parameters = function(data, page, view_number) {
    const this_form = $('form.js-wpv-filter-form-' + view_number),
    ...
    data['wpv_sort_orderby'] =
      typeof this_form.data('orderby') != 'undefined'
        ? this_form.data('orderby')
        : ''
    data['wpv_sort_order'] =
      typeof this_form.data('order') != 'undefined'
        ? this_form.data('order')
        : ''
    data['wpv_sort_orderby_as'] =
      typeof this_form.data('orderbyas') != 'undefined'
        ? this_form.data('orderbyas')
        : ''
    data['wpv_sort_orderby_second'] =
      typeof this_form.data('orderbysecond') != 'undefined'
        ? this_form.data('orderbysecond')
        : ''
    data['wpv_sort_order_second'] =
      typeof this_form.data('ordersecond') != 'undefined'
        ? this_form.data('ordersecond')
        : ''
    ...
    if (
      this_form.hasClass('js-wpv-dps-enabled') ||
      this_form.hasClass('js-wpv-ajax-results-enabled')
    ) {
      data['dps_general'] = this_form
        .find('.js-wpv-filter-trigger, .js-wpv-filter-trigger-delayed')
        .serializeArray()
    }
  ...
  self.pagination_preload_pages = function(preload_data) {
    let page = parseInt(preload_data.page, 10),
    ...
    if (preload_data.preload_pages == 'enabled') {
      ...
      while (reach < max_reach) {
        self.pagination_preload_next_page(
          preload_data.view_number,
          page,
          max_pages,
          reach
        )
        self.pagination_preload_previous_page(
          preload_data.view_number,
          page,
          max_pages,
          reach
  ...
  self.pagination_cache_current_page = function(view_number, page) {
    window.wpvCachedPages[view_number] =
      window.wpvCachedPages[view_number] || []
    let current_page_permalink, content
  ...
  self.pagination_preload_next_page = function(
    view_number,
    page,
    max_pages,
    reach
  ...
  self.pagination_preload_previous_page = function(
    view_number,
    page,
    max_pages,
    reach
    ...
    if (!window.wpvCachedPages[view_number].hasOwnProperty(previous_page)) {
      ...
      if (previous_page + 1 > 1) {
        WPViews.view_frontend_utils
          ...
          .done(function(response) {
            if (response.success) {
              window.wpvCachedPages[view_number][previous_page] =
                response.data.full
              content = $(response.data.full).find('img')
              content.each(function() {
                window.wpvCachedImages.push(this.src)
      ...
      } else if (previous_page + 1 === 1) {
        // LOAD LAST PAGE IF ON FIRST PAGE
        WPViews.view_frontend_utils
          .get_updated_query_results(
            view_number,
            max_pages,
  ...
  self.trigger_pagination = function(view_number, page) {
    ...
    if (paged_view_data.type == 'disabled' || paged_view_data.type == 'paged') {
      switch (paged_view_data.query) {
        case 'archive':
          current_page_permalink = paged_view_data['base_permalink'].replace(
            'WPV_PAGE_NUM',
            page
          )
    ...
    if (paged_view_data.effect in self.pagination_effects_conditions) {
      if (
        !self.pagination_effects_conditions[paged_view_data.effect](
          paged_view_data,
          page
        )
      ) {
    ...
    if (window.wpvCachedPages[view_number].hasOwnProperty(page)) {
      ...
      if (paged_view_data.spinner !== 'disabled') {
        if (paged_view_data.effect in self.pagination_effects_spinner) {
          self.pagination_effects_spinner[paged_view_data.effect](
            view_number,
            wpvPaginatorLayout
          )
        } else {
  ...
  self.prepare_slide = function(data) {
    let slide_data = $.extend({}, self.slide_data_defaults, data),
    ...
    slide_data.responseView = responseObj.find(
      '.js-wpv-view-layout-' + slide_data.view_number
    )
    slide_data.responseFilterSelector = responseObj.find(
      'form.js-wpv-filter-form-' + slide_data.view_number
    )
    slide_data.responseFilter = responseObj
      .find('form.js-wpv-filter-form-' + slide_data.view_number)
      .html()
    slide_data.pagination_page_permalink = slide_data.responseView.data(
      'permalink'
    )
    ...
    if (
      slide_data.wpvPaginatorLayout.hasClass('js-wpv-layout-preload-images')
    ) {
      preloadedImages = []
  ...
  self.pagination_slide = function(data) {
    const slide_data = $.extend({}, self.slide_data_defaults, data)
  ...
  self.pagination_queue_trigger = function(
    view_number,
    next,
    wpvPaginatorFilter
  ) {
    let args, page, max_pages
    if (
      window.wpvPaginationQueue.hasOwnProperty(view_number) &&
      window.wpvPaginationQueue[view_number].length > 0
    ) {
  ...
  $(document).on('click', '.js-wpv-pagination-link', function(e) {
    ...
    for (i = 1; i <= data_collected.max_pages; i++) {
      if (i === data_collected.page) {
        $('.js-wpv-page-link-' + data_collected.view_number + '-' + i).addClass(
          'wpv_page_current'
        )
      } else {
        $(
          '.js-wpv-page-link-' + data_collected.view_number + '-' + i
        ).removeClass('wpv_page_current')
      }
    }
  ...
  self.init_effects = function() {
    self.pagination_effects = {
      infinite: function(slide_data) {
        if (
          slide_data.page !=
          self.get_paged_view(slide_data.view_number).page + 1
        ) {
          // This should never happen! See self.pagination_effects_conditions
          $('.js-wpv_slide_loading_img_' + slide_data.view_number).fadeOut(
            function() {
              $(this).remove()
            }
          )
          ...
          data_for_history.pagination_page_permalink =
            slide_data.pagination_page_permalink
          ...
          if (
            ...
            slide_data.responseView
              .find('.js-wpv-loop')
              .prepend(
                slide_data.wpvPaginatorLayout.find('.js-wpv-loop').html()
              )
          ...
          slide_data.wpvPaginatorLayout
            ...
            .animate({ opacity: 1 }, 300, function() {
              ...
              window.wpvPaginationAjaxLoaded[slide_data.view_number] = true
              window.wpvPaginationAnimationFinished[
                slide_data.view_number
              ] = true
              slide_data.callback_next_func()
              self.manage_browser_history(data_for_history)
              $(document).trigger('js_event_wpv_pagination_completed', [
                data_for_events,
              ])
              self.pagination_queue_trigger(
                slide_data.view_number,
                slide_data.next,
                slide_data.wpvPaginatorFilter
              )
            })
        }
      },
      slideh: function(slide_data) {
        let height = slide_data.wpvPaginatorLayout.height(),
          ...
          if (old_height === new_height) {
            slide_data.wpvPaginatorLayout
              ...
              .animate(
                {
                  marginLeft:
                    '-' + slide_data.wpvPaginatorLayout.outerWidth() + 'px',
                },
                slide_data.speed + 500,
                function() {
                  slide_data.responseView.css({
                    position: 'static',
                    float: 'none',
                  })
                  slide_data.wpvPaginatorLayout
              ...
              .animate(
                ...
                function() {
                  slide_data.wpvPaginatorLayout
                    ...
                    .parent()
                    .animate(
                      { height: slide_data.responseView.outerHeight() + 'px' },
                      slide_data.speed / 2,
                      function() {
                        slide_data.responseView.css({
                          position: 'static',
                        ...
                        slide_data.wpvPaginatorFilter.html(
                          slide_data.responseFilter
                        )
                    ...
                    .animate(
                      {
                        marginLeft:
                          '-' +
                          slide_data.wpvPaginatorLayout.outerWidth() +
                          'px',
                      },
                      slide_data.speed + 500,
                      function() {
          ...
          slide_data.wpvPaginatorLayout
            ...
            .wrapAll(
              '<div style="height:' +
                height +
                ';width:' +
                (slide_data.responseView.outerWidth() +
                  slide_data.wpvPaginatorLayout.outerWidth()) +
                'px; margin-left:-' +
                slide_data.wpvPaginatorLayout.outerWidth() +
                'px;" />'
      ...
      slidev: function(slide_data) {
        let old_height = slide_data.wpvPaginatorLayout.outerHeight(),
          new_height,
          data_for_events = {},
          data_for_history = {}
        ...
        if (slide_data.next === true) {
          ...
          if (old_height === new_height) {
            slide_data.wpvPaginatorLayout
              ...
              .animate(
                {
                  marginTop: '-' + slide_data.responseView.outerHeight() + 'px',
                },
                slide_data.speed + 500,
                function() {
                  slide_data.responseView.css({
          ...
          slide_data.wpvPaginatorLayout
            ...
            .css({
              position: 'relative',
              'margin-top': '-' + slide_data.responseView.outerHeight() + 'px',
            })
          if (old_height === new_height) {
            slide_data.wpvPaginatorLayout
              .parent()
          ...
          if (old_height === new_height) {
            ...
            slide_data.responseView
              ...
              .fadeIn(slide_data.speed, function() {
                ...
                WPViews.view_pagination.pagination_queue_trigger(
                  slide_data.view_number,
                  slide_data.next,
                  slide_data.wpvPaginatorFilter
                )
  ...
  self.manage_browser_history = function(data) {
    if (self.get_paged_view(data.view_number).type != 'rollover') {
  ...
  window.onpopstate = function(event) {
    if (event.state == null) {
      const last_paginated_view_number = self.last_paginated_view.pop()
      if (last_paginated_view_number != undefined) {
        self.add_paginated_history = false
        self.paged_views_initial_page[last_paginated_view_number] =
          self.paged_views_initial_page[last_paginated_view_number] || 1
        self.trigger_pagination(
          last_paginated_view_number,
          self.paged_views_initial_page[last_paginated_view_number]
        )
      }
    } else {
      if (_.has(event.state, 'view_number') && _.has(event.state, 'page')) {
        self.add_paginated_history = false
  ...
  self.updateLanguageSwitcherURLs = function(permalink) {
    $('.wpml-ls-item').each(function(index, langSwitcher) {
      const parsedPermalink = self.parseURL(permalink),
        langSwitcherHref = $(langSwitcher)
          .find('a')
          .first()
          .attr('href'),
      ...
      const finalParams = $.extend(
          {},
          parsedLangSwitcherHref.params,
          parsedPermalink.params
        ),
        strippedLanguageSwitcherHref = langSwitcherHref.replace(
          parsedLangSwitcherHref.query,
          ''
        )
      ...
      $(langSwitcher)
        .find('a')
        .first()
        .attr('href', strippedLanguageSwitcherHref + '?' + $.param(finalParams))
    })
  }
  ...
  self.init_effects_spinner = function() {
    self.pagination_effects_spinner['fade'] = self.pagination_effects_spinner[
      'slideleft'
    ] = self.pagination_effects_spinner[
      'slideright'
    ] = self.pagination_effects_spinner[
      ...
      'slidedownforward'
    ] = self.pagination_effects_spinner['slidev'] = function(
      view_number,
      wpvPaginatorLayout
    ) {
      const img = new Image()
      ...
      img.onload = function() {
        const wpvPaginatorLayoutOffset = wpvPaginatorLayout.position(),
          wpvPaginatorSpinner =
            '<div style="' +
            'width:' +
            img.width +
            ...
            self.get_paged_view(view_number).spinner_image +
            ');' +
            'position:absolute;' +
            'z-index:99;' +
            'top:' +
            (Math.round(wpvPaginatorLayoutOffset.top) +
              Math.round(wpvPaginatorLayout.height() / 2) -
              Math.round(img.height / 2)) +
            'px;' +
            'left:' +
            (Math.round(wpvPaginatorLayoutOffset.left) +
              Math.round(wpvPaginatorLayout.width() / 2) -
              Math.round(img.width / 2)) +
            'px;' +
            '" ' +
            'class="wpv_slide_loading_img js-wpv_slide_loading_img_' +
            view_number +
            '"' +
            '>' +
            '</div>'
    ...
    self.pagination_effects_spinner['infinite'] = function(
      view_number,
      wpvPaginatorLayout
    ) {
      const img = new Image()
  ...
  self.init_paged_views = function(container) {
    let init_scrolling_event = false,
    ...
    if (!self.init_scrolling_event_fired && init_scrolling_event) {
      self.init_scrolling_event_callback()
    }

    if (!self.init_rollover_timing_fired && init_rollover_timing) {
  ...
  self.init_paged_view = function(view_number) {
    this.historyP = this.historyP || []
    self.paged_views[view_number] = $('#wpv-view-layout-' + view_number).data(
      'pagination'
    )
    self.paged_views_initial_page[view_number] =
      self.paged_views[view_number].page
    ...
    if (
      ...
      $('#wpv-view-layout-' + view_number).removeClass(
        'js-wpv-layout-infinite-scrolling'
      )
    }
    if (
      $('#wpv-view-layout-' + view_number).parents('.js-wpv-view-layout')
        .length > 0
    ) {
  ...
  self.is_infinite_triggable = function(view_layout) {
    ...
    return (
      flag_element.offset().top + flag_element.outerHeight() <=
        $(window).scrollTop() + $(window).height() + infinite_tolerance ||
  ...
  self.trigger_rollover = function(view_data) {
    const rollover_data = $.extend(
        {},
        { view_number: '', page: 1, force_reset: false },
        view_data
      ),
    ...
    if (
      ...
      setTimeout(function() {
        ...
        if (
          self.rollover_paused[rollover_index] &&
          ...
          if (
            ...
            var view_page = rollover_data.page,
              rollover_index = _.findIndex(self.rollover_running, function(
                item
              ) {
                return item.view_number == view_number
              })
            ...
            var rollover_index = _.findIndex(self.rollover_running, function(
              item
            ) {
              return item.view_number == view_number
            })
      ...
      const rollover_index = _.findIndex(self.rollover_running, function(item) {
        return item.view_number == view_number
      })
      self.rollover_running.splice(rollover_index, 1)
    }
  ...
  self.init_rollover_timing_callback = function() {
    ...
    const rollover_running = self.rollover_running.slice(0)
  ...
  $(document).on('mouseenter', '.js-wpv-view-layout', function() {
    const view_number = $(this).data('viewnumber')
  ...
  $(document).on('mouseleave', '.js-wpv-view-layout', function() {
    const view_number = $(this).data('viewnumber')
    const rollover_index = _.findIndex(self.rollover_paused, function(item) {
      return item.view_number == view_number
    })
...
WPViews.ViewParametricSearch = function($) {
  ...
  self.manage_update_results = function(data) {
    if (data.ajax_before !== '') {
      const ajax_before_func = window[data.ajax_before]
  ...
  self.manage_changed_form = function(settings) {
    let fil = settings.form,
      ...
      spinnerContainer = fil
        .find('.js-wpv-dps-spinner')
        .add(additional_forms.find('.js-wpv-dps-spinner')),
      ...
      if (additional_forms.length > 0) {
        ...
        if (view_type == 'form') {
          ...
          if (settings.update_results) {
            ...
            $(document).trigger('js_event_wpv_parametric_search_started', [
              data_for_events,
            ])
          }
          if (spinnerItems) {
            // TODO maybe only when updating results
          ...
          WPViews.view_frontend_utils
            .get_updated_query_results(view_num, 1, fil, ajax_get)
            .done(function(response) {
              if (response.success) {
                new_content_form = $('<div></div>').append(response.data.form)
                new_content_full = $('<div></div>').append(response.data.full)
                new_content_form_filter = new_content_form.find(
                  '.js-wpv-filter-form-' + view_num
                )
                new_content_full_filter = new_content_full.find(
                  '.js-wpv-filter-form-' + view_num
                )
                new_content_full_layout = new_content_full.find(
                  '.js-wpv-view-layout-' + view_num
                )

                fil.html(new_content_form_filter.html())
                WPViews.view_frontend_utils.destroy_frontend_datepicker()
                WPViews.view_frontend_utils.clone_form(
                  fil,
                  additional_forms_only
                )
                ...
                $(
                  document
                ).trigger('js_event_wpv_parametric_search_form_updated', [
                  data_for_events,
                ])
                if (settings.update_results) {
                  ...
                  data_for_manage_updated_results.permalink =
                    response.data['parametric_permalink']
        ...
        if (view_type == 'form') {
          ...
          WPViews.view_frontend_utils
            .get_updated_query_results(view_num, 1, fil, 'full')
            .done(function(response) {
              if (response.success) {
      ...
      if (additional_forms.length > 0) {
        ...
        if (view_type == 'form') {
          ...
          if (additional_forms_full.length > 0 || settings.update_results) {
            ...
            data_for_events.view_changed_form_additional_forms_only = additional_forms_only
            data_for_events.view_changed_form_additional_forms_full = additional_forms_full
            $(document).trigger('js_event_wpv_parametric_search_form_updated', [
              data_for_events,
            ])
          }
        } else {
  ...
  self.extend_wpv_parametric_search_triggered_data = function(data) {
    ...
    if (
      ...
      (settings.force_results_update == true ||
        ...
        _.has(
          WPViews.view_pagination.paged_views[settings.view_unique_id],
          'has_controls_in_form'
        ) &&
        WPViews.view_pagination.paged_views[settings.view_unique_id]
          .has_controls_in_form == 'enabled'
    ) {
      settings.force_form_update = true
    }

    settings.update_form =
      settings.force_form_update || settings.form.hasClass('js-wpv-dps-enabled')
    settings.update_results =
      view_loop.length > 0 &&
      (settings.force_results_update ||
        settings.form.hasClass('js-wpv-ajax-results-enabled'))
  ...
  $(document).on('click', '.js-wpv-date-display', function() {
    const url_param = $(this).data('param')
  ...
  $(document).on('change', '.js-wpv-post-relationship-update', function() {
    let thiz = $(this),
      ...
      watchers = fil
        .find('.js-wpv-' + currentposttype + '-watch')
        .add(additional_forms.find('.js-wpv-' + currentposttype + '-watch')),
    ...
    data_for_events = self.extend_wpv_parametric_search_triggered_data(
      data_for_events
    )
  ...
  $(document).on('click', '.js-wpv-reset-trigger', function(e) {
    ...
    if (fil.hasClass('js-wpv-form-only')) {
      watchers = fil
        .find('input, select')
        .add(additional_forms.find('input, select'))
      watcherslength = watchers.length
      form_only_force_update =
        fil.hasClass('js-wpv-dps-enabled') ||
        fil.find('.js-wpv-post-relationship-update').length > 0
      if (
      ...
      if (
        !form_only_results_force_update &&
        $('#wpv-view-layout-' + fil.data('viewnumber')).length > 0
      ) {
      ...
      window.location.href = WPViews.view_frontend_utils.updateUrlQuery(
        'wpv_view_count',
        null,
        target
      )
  ...
  $(document).on('js_event_wpv_parametric_search_triggered', function(
    event,
    settings
  ) {
  ...
  $(document).on('js_event_wpv_parametric_search_started', function(
    event,
    data
  ) {})
