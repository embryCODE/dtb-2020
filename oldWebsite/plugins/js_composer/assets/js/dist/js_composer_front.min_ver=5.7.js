...
(document.documentElement.className += ' js_active '),
  ...
  (function() {
    for (
      let prefix = ['-webkit-', '-moz-', '-ms-', '-o-', ''], i = 0;
      i < prefix.length;
      i++
    )
        ...
        jQuery('.vc_progress_bar').waypoint(
          function() {
            jQuery(this)
              .find('.vc_single_bar')
              .each(function(index) {
                const bar = jQuery(this).find('.vc_bar'),
  ...
  'function' != typeof window.vc_toggleBehaviour &&
    (window.vc_toggleBehaviour = function($el) {
      function event(e) {
        ...
        const element = jQuery(this).closest('.vc_toggle'),
  ...
  'function' != typeof window.vc_tabsBehaviour &&
    (window.vc_tabsBehaviour = function($tab) {
      if (jQuery.ui) {
        const $call = $tab || jQuery('.wpb_tabs, .wpb_tour'),
          ver =
            ...
            jQuery(this)
              ...
              .click(function(e) {
                if ((e.preventDefault(), old_version)) {
                  ...
                  const length = $tabs.find('.wpb_tab').length
                  ;(index = jQuery(this)
                    .parent()
                    .hasClass('wpb_next_slide')
                    ? length <= index + 1
  ...
  'function' != typeof window.vc_teaserGrid &&
    (window.vc_teaserGrid = function() {
      const layout_modes = { fitrows: 'fitRows', masonry: 'masonry' }
      jQuery(
        ...
        $thumbs.isotope({
          ...
          $container
            ...
            .click(function(e) {
              e.preventDefault()
              const $thumbs = jQuery(this).data('isotope')
              jQuery(this)
                .parent()
                .parent()
                .find('.active')
      ...
      ($parent ? $parent.find('.wpb_carousel') : jQuery('.wpb_carousel')).each(
        function() {
          ...
          if (!0 !== $this.data('carousel_enabled') && $this.is(':visible')) {
            ...
            const carousele_li = jQuery(this).find('.wpb_thumbnails-fluid li')
            carousele_li.css({
              'margin-right': carousele_li.css('margin-left'),
              'margin-left': 0,
            })
            const fluid_ul = jQuery(this).find('ul.wpb_thumbnails-fluid')
    ...
    (window.vc_slidersBehaviour = function() {
      jQuery('.wpb_gallery_slides').each(function(index) {
        ...
        if (this_element.hasClass('wpb_slider_nivo')) {
          let sliderTimeout = 1e3 * this_element.attr('data-interval')
...
function getSizeName() {
  const screen_w = jQuery(window).width()
...
function vc_accordionActivate(event, ui) {
  if (ui.newPanel.length && ui.newHeader.length) {
    const $pie_charts = ui.newPanel.find('.vc_pie_chart:not(.vc_ready)'),
    ...
    void 0 !== jQuery.fn.isotope &&
      ...
      ui.newPanel.find('.vc_masonry_media_grid, .vc_masonry_grid').length &&
        ui.newPanel
          ...
          .each(function() {
            const grid = jQuery(this).data('vcGrid')
            grid &&
              grid.gridBuilder &&
  ...
  ('function' != typeof window.wpb_prepare_tab_content &&
    (window.wpb_prepare_tab_content = function(event, ui) {
      ...
      if (
        ...
        const $frame = $google_maps.find('iframe')
...
function vc_setHoverBoxHeight(hoverBox) {
  hoverBox.each(function() {
    ...
    let frontHeight = $this.find('.vc-hoverbox-front-inner').outerHeight(),
...
function vc_prepareHoverBox() {
  const hoverBox = jQuery('.vc-hoverbox')
